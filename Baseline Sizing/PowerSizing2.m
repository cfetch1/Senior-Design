function [PW_,PWmin_,PWmin,index] = PowerSizing2(WS,CDmin,CDto,MTOW,CLmax,AR,V,ROC,h,dg,eta,plots)
...
[TW_dtod] = Desired_Takeoff_Distance(WS, V(3), 33.2, dg, q(V(3),rho(h(3))), CDto, .04, MTOW);
...
[PW_lcvt_SL] = zeros(length(WS),1);
[PW_dcr_SL] = zeros(length(WS),1);
[PW_dtod_SL] = zeros(length(WS),1);
